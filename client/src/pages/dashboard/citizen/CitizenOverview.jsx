...
import { FiFileText, FiClock, FiRefreshCw, FiCheckCircle, FiDollarSign } from 'react-icons/fi';
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted mt-0.5">Your issue reports and activity summary</p>
      </div>

      <div className="grid grid-cols-2 xl:grid-cols-5 gap-4">
        <StatCard label="Total Submitted" value={issues.length}           Icon={FiFileText}    accent="bg-primary" />
        <StatCard label="Pending"         value={pending}                  Icon={FiClock}       accent="bg-green-400" />
        <StatCard label="In Progress"     value={inProgress}               Icon={FiRefreshCw}   accent="bg-green-500" />
        <StatCard label="Resolved"        value={resolved}                 Icon={FiCheckCircle} accent="bg-green-600" />
        <StatCard label="Total Payments"  value={`kr ${totalPaid.toLocaleString()}`} Icon={FiDollarSign} accent="bg-violet-500" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <PanelCard title="Issues by Status">
          {pieData.length > 0 ? (
            ...
            <p className="text-muted text-sm text-center py-10">No issues yet. Start by reporting one!</p>
          )}
        </PanelCard>

        <PanelCard title="Recent Issues">
          {recent.length === 0 ? (
