...
import {
  FiGrid, FiList, FiPlusCircle, FiUser, FiGlobe,
...
const PAGE_TITLES = {
  ...
  '/explore':                        'Explore',
  '/dashboard/citizen/my-issues':    'My Issues',
  '/dashboard/citizen/report-issue': 'Report Issue',
  '/dashboard/citizen/profile':      'Profile',
};
    ...
    <div className="flex items-center justify-between px-5 h-16 border-b border-border flex-shrink-0">
      <Link to="/" onClick={onClose} className="flex items-center gap-2.5">
        ...
        <div>
          ...
          <span className="text-[10px] font-bold uppercase tracking-widest text-primary leading-none block mt-0.5">Citizen</span>
        </div>
      </Link>
      {onClose && (
    ...
    <div className="px-3 py-4 border-t border-border flex-shrink-0">
      <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-surface-alt transition-colors">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-muted capitalize leading-tight mt-0.5">{dbUser?.role || 'Citizen'}</p>
        </div>
        <button
          onClick={onLogout}
          title="Logout"
        ...
        <header className="h-14 flex items-center justify-between px-4 sm:px-6 border-b border-border bg-surface/80 backdrop-blur-sm flex-shrink-0 gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[11px] text-muted leading-none mt-0.5 hidden sm:block">Citizen Dashboard</p>
            </div>
          </div>
